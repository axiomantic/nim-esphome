...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  if installer.enableSetupGuide:
    ...
    html.add("              <div style=\"margin-top: 6px;\"><strong>⚠️ If Home Assistant prompts for an Encryption Key:</strong><br/>Brand-new boards ship with Seeed's encrypted factory firmware. If Home Assistant discovered the board before it was flashed, it cached a requirement for an encryption key and will reject a blank field with <em>\"not all required fields are filled in\"</em>.</div>")
    html.add("              <div style=\"margin-top: 6px;\"><strong>👉 How to Fix in 10 Seconds:</strong> Cancel the prompt, click <strong>Add Integration &rarr; ESPHome</strong>, and enter your device IP (or <code>esphome-satellite-&lt;mac&gt;.local</code>) and port <code>6053</code>. It connects directly with zero encryption prompts!</div>")
    html.add("            </div>")
    if installer.homeAssistantDomain.len > 0:
      html.add("            <div style=\"display: flex; gap: 8px; flex-wrap: wrap; margin: 10px 0;\">")
  ...
  if installer.enableSuccessModal:
    ...
    html.add("                <div style=\"margin-top: 4px;\"><strong>⚠️ If prompted for an Encryption Key:</strong> Brand-new boards ship with Seeed's encrypted firmware. If Home Assistant saw the board before it was flashed, it cached that requirement and will reject blank entries with <em>\"not all required fields are filled in\"</em>.</div>")
    html.add("                <div style=\"margin-top: 4px;\"><strong>👉 10-Second Fix:</strong> Cancel the prompt, go to <strong>Add Integration &rarr; ESPHome</strong>, and enter your device IP (or <code>esphome-satellite-&lt;mac&gt;.local</code>) and port <code>6053</code> to connect directly!</div>")
    html.add("              </div>")
    html.add("            </div>")
    html.add("          </div>")
    html.add("          <div class=\"next-step-item\">")
