...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  html.add("    .trained-success-badge { margin-top: 10px; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 6px; padding: 8px 12px; font-size: 0.8rem; color: #34d399; display: flex; align-items: center; justify-content: space-between; gap: 8px; }")
  html.add("    .download-model-btn { background: #064e3b; color: #a7f3d0; border: 1px solid #059669; padding: 4px 8px; border-radius: 4px; font-size: 0.72rem; text-decoration: none; font-weight: 600; cursor: pointer; white-space: nowrap; }")
  html.add("    .download-model-btn:hover { background: #047857; }")
  ...
  for field in installer.fields:
    if field.kind == ifkWakeWordSlots:
      ...
      html.add("                <a class=\"download-model-btn\" download=\"custom_wake_word.tflite\">⬇ Download .tflite</a>")
      html.add("              </div>")
      html.add("            </div>")
