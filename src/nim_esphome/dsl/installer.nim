...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  if installer.enableSetupGuide:
    ...
    html.add("            <p>Click <strong>Configure</strong>, then click <strong>Submit</strong>. This firmware connects without an encryption key. If Home Assistant prompts for an <strong>Encryption Key</strong>, it is detecting a pre-existing cached discovery or factory firmware — cancel that prompt and add the device via <strong>Add Integration &rarr; ESPHome</strong> using its IP or hostname.</p>")
    if installer.homeAssistantDomain.len > 0:
      html.add("            <div style=\"display: flex; gap: 8px; flex-wrap: wrap; margin: 10px 0;\">")
  ...
  if installer.enableEraseButton:
    ...
    html.add("          eraseStatus.innerHTML = ICONS.check + ' <strong>Flash erased successfully!</strong> All flash partitions and settings wiped.<br/><small style=\"display:block; margin-top:6px; opacity:0.85;\">ℹ️ <em>Note: If you connect serial logs now, you will see <code>invalid header: 0xffffffff</code> repeating—this is normal when flash memory is empty!</em> If flashing does not start immediately, hold the <strong>BOOT</strong> button, tap <strong>RST</strong>, then release BOOT.</small><br/>You can now click <strong>Install Firmware</strong> above to flash clean firmware.';")
    html.add("        } catch (err) {")
  ...
  if installer.enableSuccessModal:
    ...
    html.add("            <div>")
    html.add("              <strong>Add in Home Assistant:</strong> Open Home Assistant &rarr; <strong>Settings &rarr; Devices &amp; Services</strong>. Look under <strong>Discovered</strong> for <strong>" & installer.title & "</strong> and click <strong>Configure</strong>.")
    html.add("              <div class=\"subtext\" style=\"background: rgba(59, 130, 246, 0.12); border-left: 3px solid #3b82f6; padding: 6px 10px; margin-top: 6px; border-radius: 4px; color: #93c5fd;\">")
    html.add("                <strong>🔑 Encryption Key Prompt:</strong> This firmware connects without an encryption key. If Home Assistant prompts for one, it is reading a cached discovery flow from Seeed's factory firmware. Simply dismiss/cancel that prompt and add via <strong>Add Integration &rarr; ESPHome</strong> using its IP or <code>.local</code> address.")
    html.add("              </div>")
    html.add("            </div>")
    html.add("          </div>")
    html.add("          <div class=\"next-step-item\">")
