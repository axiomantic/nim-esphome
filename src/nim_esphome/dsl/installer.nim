...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  if installer.enableSetupGuide:
    ...
    html.add("            <p>Click <strong>Configure</strong>, then click <strong>Submit</strong>. If Home Assistant prompts for an <strong>Encryption Key</strong>, simply <strong>leave it blank</strong> (no key is required).</p>")
    if installer.homeAssistantDomain.len > 0:
      html.add("            <div style=\"display: flex; gap: 8px; flex-wrap: wrap; margin: 10px 0;\">")
  ...
  if installer.enableSuccessModal:
    ...
    html.add("                <strong>🔑 Encryption Key Prompt:</strong> If Home Assistant shows an <em>Encryption key</em> field, simply <strong>leave it blank</strong> and click <strong>Submit</strong>. This firmware does not require an encryption key.")
    html.add("              </div>")
    html.add("            </div>")
    html.add("          </div>")
    html.add("          <div class=\"next-step-item\">")
