...
proc renderFieldBody(html: var seq[string], field: InstallerField, installer: InstallerDefinition) =
  ...
  of ifkCustomWakeWordSlots:
    ...
    for i in 0 ..< field.maxSlots:
      ...
      html.add("              <div class=\"custom-wake-inputs\" style=\"margin-top: 8px;\">")
      html.add("                <label style=\"font-size: 0.76rem; color: #94a3b8; display: block; margin-bottom: 4px;\">Wake Word Name / Phrase</label>")
      html.add("                <input type=\"text\" id=\"slotPhrase_" & field.name & "_" & $i & "\" placeholder=\"e.g. Jarvis\" style=\"width: 100%; box-sizing: border-box;\">")
      html.add("              </div>")
...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  html.add("    .actions { margin-top: 28px; display: flex; flex-direction: column; align-items: center; gap: 10px; width: 100%; }")
  html.add("    .action-primary { width: 100%; }")
  html.add("    .action-primary esp-web-install-button { width: 100%; display: block; }")
  html.add("    button.install-btn { background: var(--primary); color: white; border: none; padding: 14px 28px; font-size: 1.05rem; font-weight: 600; border-radius: 8px; cursor: pointer; transition: background 0.2s; width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px; white-space: nowrap; box-sizing: border-box; }")
  html.add("    button.install-btn:hover { background: var(--primary-hover); }")
  html.add("    .action-secondary-row { width: 100%; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; }")
  html.add("    .action-secondary-row improv-wifi-serial-launch-button { width: 100%; display: block; }")
  html.add("    button.wifi-btn { background: #0284c7; color: white; border: 1px solid #0369a1; padding: 11px 14px; font-size: 0.88rem; font-weight: 600; border-radius: 8px; cursor: pointer; transition: all 0.2s ease; width: 100%; display: inline-flex; align-items: center; justify-content: center; gap: 7px; white-space: nowrap; box-sizing: border-box; }")
  html.add("    button.wifi-btn:hover { background: #0369a1; }")
  html.add("    button.wifi-btn span, .ha-btn span { white-space: nowrap; }")
  html.add("    .modal-wifi-btn { background: #0284c7; color: white; border: none; padding: 8px 14px; font-size: 0.82rem; font-weight: 600; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; white-space: nowrap; }")
  html.add("    .modal-wifi-btn:hover { background: #0369a1; }")
  html.add("    .btn-guide-wifi { background: #0284c7; color: white; border: none; padding: 8px 14px; font-size: 0.82rem; font-weight: 600; border-radius: 6px; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; white-space: nowrap; }")
  html.add("    .btn-guide-wifi:hover { background: #0369a1; }")
  html.add("    .ha-btn { background: #1e293b; color: #38bdf8; border: 1px solid #0284c7; padding: 11px 14px; font-size: 0.88rem; font-weight: 600; border-radius: 8px; cursor: pointer; text-decoration: none; transition: all 0.2s ease; display: inline-flex; align-items: center; justify-content: center; gap: 7px; white-space: nowrap; box-sizing: border-box; width: 100%; }")
  html.add("    .ha-btn:hover { background: rgba(2, 132, 199, 0.2); border-color: #38bdf8; color: #ffffff; }")
  html.add("    .btn-guide-ha { background: #1e293b; color: #38bdf8; border: 1px solid #0284c7; padding: 8px 14px; font-size: 0.82rem; font-weight: 600; border-radius: 6px; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s ease; white-space: nowrap; }")
  html.add("    .btn-guide-ha:hover { background: rgba(2, 132, 199, 0.2); border-color: #38bdf8; color: #ffffff; }")
  html.add("    .action-utility-row { width: 100%; display: flex; justify-content: center; margin-top: 2px; }")
  html.add("    .erase-btn { background: transparent; color: #94a3b8; border: 1px solid #334155; padding: 7px 16px; font-size: 0.82rem; font-weight: 500; border-radius: 6px; cursor: pointer; transition: all 0.2s ease; display: inline-flex; align-items: center; justify-content: center; gap: 6px; white-space: nowrap; }")
  html.add("    .erase-btn:hover { background: rgba(239, 68, 68, 0.1); border-color: #ef4444; color: #fca5a5; }")
  html.add("    .erase-btn svg { opacity: 0.75; }")
  html.add("    .erase-btn:hover svg { opacity: 1; }")
  html.add("    .erase-btn span { white-space: nowrap; }")
  html.add("    .install-nvs-tip { margin-top: 4px; font-size: 13px; color: #94a3b8; text-align: center; line-height: 1.4; max-width: 520px; }")
  ...
  html.add("    @media (max-width: 480px) {")
  html.add("      .action-secondary-row { grid-template-columns: 1fr; }")
  html.add("    }")
  ...
  html.add("      <div class=\"action-primary\">")
  ...
  html.add("      </div>")
  if installer.enableImprovWifi or installer.homeAssistantDomain.len > 0:
    html.add("      <div class=\"action-secondary-row\">")
    if installer.enableImprovWifi:
  ...
  if installer.enableEraseButton:
    html.add("      <div class=\"action-utility-row\">")
    ...
    html.add("        </button>")
    html.add("      </div>")
  html.add("      <div class=\"install-nvs-tip\">Updating an existing device? Leave <strong>Erase device</strong> unchecked in the install popup to preserve your Wi-Fi settings, volume preferences, wake words, and sound themes.</div>")
  ...
  for field in installer.fields:
    ...
    if field.kind == ifkCustomWakeWordSlots:
      ...
      html.add("    const slotCutoffs_" & field.name & " = [];")
      html.add("    for (let i = 0; i < " & $field.maxSlots & "; i++) { slotBuffers_" & field.name & ".push(null); slotFileNames_" & field.name & ".push(''); slotCutoffs_" & field.name & ".push('0.40'); }")
      html.add("")
      ...
      html.add("              slotCutoffs_" & field.name & "[slotIdx] = '0.40';")
      ...
      html.add("        return;")
      html.add("      }")
      html.add("      const cutoff = slotCutoffs_" & field.name & "[slotIdx] || '0.40';")
      ...
      for i in 0 ..< field.maxSlots:
        ...
        html.add("          slotCutoffs_" & field.name & "[" & $i & "] = '0.40';")
        ...
        html.add("            }")
        html.add("            if (meta.micro && meta.micro.probability_cutoff) {")
        html.add("              slotCutoffs_" & field.name & "[" & $i & "] = String(meta.micro.probability_cutoff);")
        html.add("            }")
        html.add("          } catch (err) {")
