...
proc generateHtml*(installer: InstallerDefinition): string =
  ...
  if installer.enableTerminalConsole:
    ...
    html.add("      const textDecoder = new TextDecoderStream();")
    html.add("      const readableStreamClosed = termPort.readable.pipeTo(textDecoder.writable);")
    html.add("      termReader = textDecoder.readable.getReader();")
    html.add("      let buffer = '';")
    html.add("      try {")
    ...
    html.add("      } finally {")
    html.add("        try { await termReader.cancel(); } catch (_) {}")
    html.add("        try { await readableStreamClosed.catch(() => {}); } catch (_) {}")
